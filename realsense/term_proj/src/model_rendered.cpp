...
//	rs_main(argc, argv);///////////
    ...
    GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "DIP2016 HARP", nullptr, nullptr); // Windowed
	...
	//Model ourModel(FileSystem::getPath("resources/objects/planet/planet.obj").c_str());
    // Draw in wireframe
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	if (key < 0) return;//boundary check
